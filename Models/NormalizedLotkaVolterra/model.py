"""
	...
	model.py:
		...
		This file contains the normalized Lotka-Volterra model. 
...
def uncompressParams(params, kwargs={}): 
	...
	return (nVars, np.array(r), np.array(alpha)); 
...
def unpackParams(params, kwargs={}): 
	...
	return uncompressParams(params); 
...
def initializeParams(kwargs={}): 
	...
	for iVar in range(nVars): 
		...
		for jVar in range(iVar+1, nVars): 
			params["alpha_"+str(iVar)+'_'+str(jVar)] = np.random.uniform(0, 2); 
			params["alpha_"+str(jVar)+'_'+str(iVar)] = np.random.uniform(0, 2); 
...
def F(x, params, kwargs={}): 
	"""	F funciton: 
			...
			This function implements the differential step for the normalized Lotka-Volterra model. (ACHTUNG!! Find some
			references.) 
	...
	(nVars, r, alpha) = uncompressParams(params); 
	r_ = np.repeat([r], x.shape[0], axis=0);

	# Computing F for each of the variables: 
	F_ =  np.multiply(r_, np.multiply(x, 1 - np.dot(alpha, x.transpose()).transpose())); 
...
def dF_r(x, params, kwargs={}): 
	...
	dF_r_merged = np.multiply(x, 1 - np.dot(alpha, x.transpose()).transpose()); 
	dF_r_ = {}; 
	for iVar in range(nVars): 
...
def dF_alpha(x, params): 
	...
	for iVar in range(nVars): 
		...
		for jVar in range(iVar+1, nVars): 
			...
			dF_alpha_["alpha_"+str(iVar)+'_'+str(jVar)][:, iVar] = -np.multiply(r_[:, iVar], np.multiply(x[:, iVar], x[:, jVar])); 

			dF_alpha_["alpha_"+str(jVar)+'_'+str(iVar)] = np.zeros(x.shape); 
			dF_alpha_["alpha_"+str(jVar)+'_'+str(iVar)][:, jVar] = -np.multiply(r_[:, jVar], np.multiply(x[:, jVar], x[:, iVar])); 
